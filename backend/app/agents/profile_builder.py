...
async def scrape_urls_node(state: ProfileBuilderState) -> dict:
    ...
    for url, data in scraped.items():
        if "error" in data:
            print(f"   {url}: {data['error']}")
            scrape_errors.append(f"Scraping failed for {url}: {data['error']}")
        else:
            print(f"   {url}: scraped successfully ({data['type']})")
        ...
        for url, data in scraped_data.items():
            if isinstance(data, dict) and "error" not in data:
                ...
                if data.get("page_content"):
                    ...
                    # Aggressively truncate page content to keep prompt manageable
                    # for free models with limited context windows
